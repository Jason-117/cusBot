...
bot.command("start", async (ctx) => {
    if (ctx.from?.id == admin_id) {
        ...
        await ctx.reply("您好,点击下方按钮可跳转至官方链接。\n（双向请回复‘人工’，我们会为您安排专属客服提供1对1支持❤）\n Hello, click the button below to go to the official link.If you encounter sending restrictions, please reply with “人工,” and we’ll assign a dedicated customer service representative to provide you with one-on-one support ❤) ", { reply_markup: menu });
    }
});
...
bot.on("message", async (ctx) => {
    ...
    if (userId !== admin_id) {
        ...
        if (!messageToAdmin) {
            ctx.reply("如需客服帮助，请回复'人工' \n If you need customer service assistance, please reply with '人工'");
            return;
        }
